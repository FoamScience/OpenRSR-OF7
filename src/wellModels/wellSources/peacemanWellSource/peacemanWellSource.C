...
:
    wellSource<RockType, 2>(name, attachedPhase, wellSourceDict),
    h_(),
    estimatedH_(false)
    re_(),
    J_()
...
void peacemanWellSource<RockType>::estimateCellSizes(const labelList cellIDs)
{
    ...
    forAll(h_, ci)
    {
        ...
        const labelList& cellEdges = this->mesh.cellEdges()[cellID];
...
) const
{
    ...
    forAll(coeff0, ci)
    {
        coeff0[ci] = srcProps.wellIndex()[ci] * kr[ci] / this->mu[ci];
    }
}
